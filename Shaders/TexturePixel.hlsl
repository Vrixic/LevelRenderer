...
float4 CalcDirectionalLight(float3 lightDir, float3 surfaceNormal, float3 pixelPositionWorld, float4 diffuseColor)
{
    ...
    return LightRatio * SceneData[0].LightColor * diffuseColor + ReflectedLight;
}

float4 CalcPointLight(uint pointLightIndex, float3 pixelPositionWorld, float3 surfaceNormal, float4 diffuseColor)
{
    float3 DistanceFromPixel = SceneData[0].Lights[pointLightIndex].Position.xyz - pixelPositionWorld;
    float3 PointLightDir = normalize(DistanceFromPixel);
    float LightRatio = saturate(dot(PointLightDir, surfaceNormal)) * SceneData[0].Lights[pointLightIndex].Position.w;
    
    float Attenuation = 1.0f - saturate(length(DistanceFromPixel) / SceneData[0].Lights[pointLightIndex].Color.w);
    Attenuation *= Attenuation;
    
    return Attenuation * LightRatio * float4(SceneData[0].Lights[pointLightIndex].Color.xyz * diffuseColor.xyz, 1);
}
...
float4 main(PixelIn input) : SV_TARGET
{
    ...
    float4 Result = CalcDirectionalLight(LightDirection, SurfaceNormal, input.PositionWorld,  DiffuseColor);
    
    for (uint i = 0; i < (uint)SceneData[0].NumOfLights; i++)
    {
